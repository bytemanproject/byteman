...
returns true. This is to allow tracing of rule IF clauses by `AND` ing a
...
to be resolved aginst the values and types available in the injection
...
you employ a module system like JBoss Modules or OSGi then you ay not
...
annotations whcih would lead to the TX module being auto-imported.
...
written at top level (outisdeof rule scope) accumulating a list of
